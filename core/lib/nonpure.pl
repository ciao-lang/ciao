...
:- use_module(engine(basic_props)). % TODO: only in assertions
...
:- use_module(engine(hiord_rt), [call/1]). % TODO: not by default
:- use_module(engine(debugger_support), [srcdbg_spy/7]). % TODO: not by default? (except if internal)
