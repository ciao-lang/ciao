:- module(c_itf_internal,
	[% (process_file)
	 ...
	 false/1,
	 processed/2,
	 % (translations for compiler_pass)
	 ...
	 uses/2, adds/2, includes/2, loads/2,
	 imports_all/2, imports_pred/7,
	 uses_file/2,
	 cleanup_itf_cache/0,
	 % (other data coming from read_record_file/4)
	 clause_of/7, defines_pred/3, dyn_decl/4, meta_pred/4,
	 imports_nocheck/4, package/2,
	 % (read_record_file data)
	 exports_pred/3,
	 module_error/1,
	 % (interface from assrt_lib, ciaopp)
	 comp_defines/1, defines/5,
	 restore_defines/5,
	 restore_imports/5,
	 restore_multifile/4,
	 % (data for mexpand)
	 defines/3, imports/5, meta_args/2, multifile/3,
	 % (read_record_file temporary data)
	 discontiguous/3,
	 % files<->module mapping
	 base_name/2, file_data/3,
	 compute_base_name/4, module_from_base/2,
	 % (default operators)
	 define_ops/0,
	 % (interface from term/sentence expanders)
	 ...
	 opt_suffix/2,
	 % (default packages)
	 default_package/1, default_shell_package/1,
	 % (compilation error handler)
	 handle_exc/1,
	 % (dynamic incore)
	 multifile/1, % (defined as multifile/2 with addmodule)
	 use_mod/3, static_base/1, static_module/1, module_loaded/4,
	 use_mod_user/2,
	 unload_mod/1,
	 interpret_file/1, interpret_module/1, interpret_srcdbg/1,
	 pred_module/2,
	 % (Compilation to .po and .wam)
	 make_object1/2,
	 make_po_file/1,
	 % (module unload)
	 abolish_module/1
