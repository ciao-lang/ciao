...
:- multifile 'tabling_rt:call_domain_projection'/2.
:- multifile 'tabling_rt:answer_domain_projection'/2.
:- multifile 'tabling_rt:current_store'/1.
:- multifile 'tabling_rt:call_store_projection'/3.
:- multifile 'tabling_rt:answer_store_projection'/3.
:- multifile 'tabling_rt:call_entail'/4.
:- multifile 'tabling_rt:answer_check_entail'/5.
:- multifile 'tabling_rt:reinstall_store'/3.
:- multifile 'tabling_rt:apply_answer'/3.
...
:- trust pred abolish_all_tables + foreign_low(abolish_all_tables_c)
   # "Removes all tables currently in the system and frees all the
   memory held by Ciao for these structures. Predicates that have been
   declared as tabled remain so, but any information in their tables
   is deleted. @pred{abolish_all_tables/0} works directly on the
   memory structures allocated for table space. This makes it very
   fast for abolishing a large volume of tables.".

%% tabled_call/1 and new_answer/0 standard predicate (without attributes)
:- trust pred tabled_call(+Call) :: cgoal + foreign_low(tabled_call_c)
   # "@pred{tabled_call/1} instruments calls to the tabled predicate
   @var{Call}. It checks if the call is a generator or a consumer
...
:- trust pred lookup_trie(+Call, -SF, -Root) :: cgoal * int * int + foreign_low(lookup_trie_c)
   # "@pred{look_up_trie/3} instruments calls to the tabled predicate
   @var{Call}. It checks if the call is a generator or a consumer
   using a trie structure. @var{SF} is unified with the substitution
   factor of @var{Call} and @var{Root} is unified with the trie node
   of @var{Call}.".

:- trust pred execute_call(+Call, +SF, +Node, -LNodePrune)
   :: cgoal * int * int * list + foreign_low(execute_call_c)
   # "@pred{execute_call/4} executes a tabled call and prunes
   @var{LNodePrune} generators.  @var{SF} is the substitution factor
   of @var{Call} and @var{Root} is the trie node of @var{Call}.".

:- trust pred consume_answer(+SF, +Node, -Space, -AttrVars)
   :: int * int * int * int + foreign_low(consume_answer_c)
   # "@pred{consume\_answer/4} consumes answers from the
   table. @var{SF} is the substitution factor of the tabled call,
   @var{Node} is the tabled call frame. @var{Space} is a pointer to
   the answer constraint store and @var{AttrVars} are the attributed
   variables of the tabled goal.".

:- trust pred new_answer_attr + foreign_low(new_answer_attr_c)
   # "@pred{new\_answer\_attr/0} looks for a previous answer which is
...
:- trust pred '$gen_tree_backtracking' + foreign_low(gen_tree_backtracking_c)
...
:- trust pred reinstall_gen_space(+SF, +CallSpace) :: int * int + foreign_low(reinstall_gen_space_c)
   # "Looks up a constrain tabled call.".

:- trust pred consume_attr_answer(+AnsSpace, +AttrsVars) :: int * int + foreign_low(consume_attr_answer_c)
   # "Looks up a constrain tabled call.".

:- trust pred set_tabling_flag(-Flag,-Mode) :: atm * atm + foreign_low(set_tabling_flag_c).
:- trust pred current_tabling_flag(-Flag,+Mode) :: atm * atm + foreign_low(current_tabling_flag_c).
...
%% :- extra_compiler_opts(['-DDEBUG_ALL']). % (enable debug trace)
%% :- extra_compiler_opts(['-DDEBUG_EXEC']).
%% :- extra_compiler_opts(['-DTRACE_REG']).

% :- extra_compiler_opts(['-g']). % (include debugging information)
