...
:- doc(module, "This module provides a simple interface for download
        of files from the Web.  It supports HTTP, HTTPS, and FTP
        protocols. 

        As backend the module uses the command line tool @tt{wget} or
        @tt{curl}, depending on the one currently available on the
         system.  Proxy are supported through the environment
        variables, @tt{http_proxy}, @tt{https_proxy}, @tt{ftp_proxy},
        and @tt{no_proxy}. Refer to the manual page of @tt{wget} or
        @tt{curl} for more information about how setting those
        variables.").
...
curl(URL, Output):-
	process_call(path(curl), ['-s', URL], [stdout(Output)]).
