...
eng_create_init(Eng) :-
	F = ~path_concat(~bld_eng_path(cdir, Eng), 'eng_static_mod.c'),
...
eng_config_sysdep(Eng) :-
	...
	EngDir = ~bld_eng_path(engdir, Eng),
	EngCfg = ~eng_cfg(Eng),
	EngMetaSh = ~path_concat(~bld_eng_path(cfgdir, Eng), 'meta_sh'),
...
gen_eng_version_h(VerBundle, Eng) :-
	H = ~path_concat(~bld_eng_path(hdir, Eng), ~eng_h_alias(Eng)),
...
gen_eng_version_c(VerBundle, Eng) :-
	VerC = ~path_concat(~bld_eng_path(cdir, Eng), 'version.c'),
...
build_engine_sh := Path :-
	...
	Path = ~path_concat(CiaoSrc, 'builder/sh_src/build_engine.sh').
