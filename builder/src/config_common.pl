...
verbose_build := ~get_bundle_flag(builder:verbose_build). % (~root_bundle)
...
with_docs := ~get_bundle_flag(builder:with_docs) :- !, '$bundle_id'(lpdoc). % (~root_bundle)
with_docs := no.
...
:- export(instype/1).
instype := ~get_bundle_flag(builder:instype). % (~root_bundle)

:- export(instciao_prefix/1).
% Prefix for installation directories
% (e.g., if /usr/local, binaries will be installed in /usr/local/bin)
instciao_prefix := ~get_bundle_flag(builder:install_prefix). % (~root_bundle)

:- export(instciao_bindir/1).
% Directory for installation of binaries
% (default <prefix>/bin)
instciao_bindir := ~get_bundle_flag(builder:install_bindir). % (~root_bundle)

% Base directory for the Ciao store dir (for bundles)
% (default <prefix>/lib)
instciao_libdir := ~get_bundle_flag(builder:install_libdir). % (~root_bundle)
